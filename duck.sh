...
install_theme() {
    ...
    if [ -e "$custom_macsploit_theme" ]; then
        mv "$custom_macsploit_theme" "$theme-tomorrow_night"
        mv "$theme-tomorrow_night" "$theme_directory"
...
if [ "$smode" == "C" ] || [ "$smode" == "c" ]; then
    ...
    css_code=$(generate_css "$bgimg" "$txtclr" "$stxtclr" "$txtsha")
    echo "$css_code" > "$HOME/Downloads/custom_macsploit_theme.js"
